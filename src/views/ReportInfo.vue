<template>
  <div class="main">
    ...
    <div class="container">
    <div class="image">
      <img :src="'http://91.222.238.181/reports/' + reports.violations_image">
    </div>
      <div class="info">
...
export default {
  ...
  methods: {
    async fetchData() {
      const res = await axios.get("http://91.222.238.181/reports/" + this.$route.params.id, {
        headers: {
    ...
    async download() {
      if (parseInt(localStorage.getItem('user_role')) === 2 || parseInt(localStorage.getItem('user_role')) === 3) {
        ...
        axios
            .get("http://91.222.238.181/reportsAct/" + this.$route.params.id, {
              headers: {
                'Authorization': 'Bearer ' + this.$store.state.token
              },
              responseType: 'blob'
    ...
    async deleteVio() {
      if (parseInt(localStorage.getItem('user_role')) === 2 || parseInt(localStorage.getItem('user_role')) === 3) {
        ...
        await axios.delete("http://91.222.238.181/reports/" + this.$route.params.id, {
          headers: {
            'Authorization': `Bearer ${this.$store.state.token}`
          }
        })
    ...
    async changeStatusVio() {
      await axios.patch("http://91.222.238.181/reports/" + this.$route.params.id, {status: 'Устранено'}, {
        headers: {
          'Authorization': `Bearer ${this.$store.state.token}`
        }
      })
    ...
    async sendEmail() {
      if (parseInt(localStorage.getItem('user_role')) === 2 || parseInt(localStorage.getItem('user_role')) === 3) {
        ...
        if (email) {
          try {
            await axios.post(`http://91.222.238.181/reports/${this.$route.params.id}/sendEmail`, {email}, {
              headers: {
                'Authorization': `Bearer ${this.$store.state.token}`
              }
            });
